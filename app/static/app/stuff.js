...
$(window).scroll(function (e) {
  if (header.offset().top !== 0) {
    if (!header.hasClass('shadowHeader')) {
...
var background = document.querySelectorAll('.background'), i;
...
document.addEventListener('scroll', () => {
  ...
  if (scrollY !== 0) {
    for (i = 0; i < background.length; ++i) {
      background[i].style.backgroundPosition = `calc(50% + ${scrollY}px) calc(50% + ${scrollY}px)`;
    }
  } else {
    for (i = 0; i < background.length; ++i) {
      background[i].style.backgroundPosition = '';
    }
  }
});
